...
class Trainer():
    ...
    def train(self, input, real_val, extreme_loss):
        ...
        if extreme_loss == True:
            # print("EXTREME LOSS USED")
            loss, mape, rmse = util.calc_metrics_extreme(predict.squeeze(1), real_val, null_val=0.0) # Loss here is the extreme value
            loss.backward()
    ...
    def eval(self, args, input, real_val):
        ...
        if args.extreme_loss == True:
            loss, mape, rmse = [x.item() for x in util.calc_metrics_extreme(predict, real, null_val=0.0)]
